...
execute if data storage bracken:config {ability_books_on: true} if entity @s[level=..6] run tellraw @s {"translate":"You do not have enough levels to cast this spell."}
execute if data storage bracken:config {ability_books_on: true} if entity @s[level=7..] run function bracken:ability_books/adamantine_will/adamantine_will
