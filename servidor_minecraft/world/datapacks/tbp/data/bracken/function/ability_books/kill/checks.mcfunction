...
execute if data storage bracken:config {ability_books_on: true} if entity @s[level=..9] run tellraw @s {"translate":"You do not have enough levels to cast this spell."}
execute if data storage bracken:config {ability_books_on: true} if entity @s[level=10..] run function bracken:ability_books/kill/kill
